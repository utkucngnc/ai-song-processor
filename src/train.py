...
from src.data.train_dataset import MusdbDataset
...
def spectrogram_loss(masked_target: th.Tensor, original: th.Tensor) -> th.Tensor:
    """
    ...
    """
    square_difference = th.square(masked_target - original)
    loss_value = th.mean(square_difference)
    return loss_value
...
def train(
    ...
    model = Isolator(stem_names=[s for s in train_dataset.targets])
    optimizer = AdamW(
        model.parameters(),
        lr=1e-3,
        eps=1e-8,
    ...
    while global_step < num_train_steps:
        ...
        for batch_idx, batch in enumerate(batch_iterator):
            ...
            for name, masked_stft in predictions.items():
                target_stft, _ = model.computeSTFT(y_target_wavs[name].squeeze())
            ...
            total_loss = (
                th.sum(th.stack(stem_losses)) / gradient_accumulation_steps
            )
